...
void gsLevelCoefsRefresh(const DeviceLduView& A, const DeviceGaussSeidelLevels& lv, GSLevelCoefs& lc)
{
    ...
    lc.bwd.resize(nE);
    if (nE <= 0) return;
    // GB10 crossover: fusion wins through the 420,600-entry duct case; 612,800 entries regressed.
    if (nE <= 450000)
    {
        gsPermuteCoefBothK<<<nBlkG(nE), TPB_G>>>(nE, lv.fwdEntSrc.data(), lv.bwdEntSrc.data(),
            A.lower, A.upper, nF, lc.fwd.data(), lc.bwd.data());
    }
    else
    {
...
void gsLevelCellsRefresh(const DeviceLduView& A, const DeviceBuffer<scalar>& b, const DeviceGaussSeidelLevels& lv, GSLevelCells& cc)
{
    ...
    if (nC <= 0) return;
    // GB10 crossover: fusion wins at 102,400 cells; 153,600 cells regressed.
    if (nC <= 110000)
    {
        gsPermuteCellBothK<<<nBlkG(nC), TPB_G>>>(nC, lv.fwdCells.data(), lv.bwdCells.data(),
            A.diag, b.data(), cc.fwdDiag.data(), cc.bwdDiag.data(), cc.fwdB.data(), cc.bwdB.data());
    }
    else
    {
